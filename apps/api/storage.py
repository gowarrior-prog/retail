...
def convert_to_webp(image_bytes: bytes, quality: int = 80) -> bytes:
    """Converts input image bytes to optimized WebP format."""
    image = Image.open(io.BytesIO(image_bytes))
    
    # Convert RGBA or Palette to RGB if necessary for saving
...
async def upload_product_image_with_fallback(file_bytes: bytes, filename: str = "") -> dict:
    """
    1. Converts image to WebP.
    2. Uploads to Supabase DB1 Bucket.
    3. If DB1 storage fails or is full, falls back to Supabase DB2 Bucket.
    """
    webp_bytes = convert_to_webp(file_bytes)
    unique_name = f"{uuid.uuid4().hex}.webp"
    
    # Try Bucket 1 (Supabase DB1)
    ...
    if client_1:
        try:
            ...
            print(f"DB1 Storage upload failed or full ({e1}), failing over to DB2 Storage...")

    # Fallback to Bucket 2 (Supabase DB2)
    ...
    return {
        "status": "error",
        "message": "Both DB1 and DB2 storage buckets failed or unconfigured.",
