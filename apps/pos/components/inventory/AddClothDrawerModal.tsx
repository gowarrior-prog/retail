...
interface AddClothDrawerModalProps {
  onClose: () => void;
}
...
export default function AddClothDrawerModal({ onClose }: AddClothDrawerModalProps) {
  ...
  const [category, setCategory] = useState('Cotton Latha');
  const [unit, setUnit] = useState('');
  const [sku, setSku] = useState(`SKU-${Math.floor(1000 + Math.random() * 9000)}`);
  const [costPrice, setCostPrice] = useState<number>(4500);
  const [retailPrice, setRetailPrice] = useState<number>(6500);
  ...
  const markupMargin = retailPrice > 0 ? ((profitPerUnit / retailPrice) * 100).toFixed(1) : '0';
  ...
  const handleCreateBarcodeSticker = () => {
    if (!sku) setSku(`SKU-${Math.floor(1000 + Math.random() * 9000)}`);
  ...
  const handleSaveCloth = async () => {
    ...
    const newProd = {
      name: title.trim(), price: retailPrice, cost_price: costPrice, category,
      sku, barcode: sku, stock, image_url: imageUrl || null, description: `${unitText}${shelfLocation || 'Main Store'}`
    };
    ...
    try {
      ...
      addProduct(res);
      handleAnimatedClose();
      showCatalogToast('✓ Product Added to Catalog', 'add');
    } catch {
      addProduct({ id: `local-${Date.now()}`, ...newProd } as any);
      handleAnimatedClose();
      showCatalogToast('✓ Product Added to Catalog', 'add');
    } finally {
  ...
  return (
    <>
          ...
          <div className="p-4 flex-1 overflow-y-auto flex flex-col gap-3 min-h-0">
              ...
              <div className="relative border-2 border-dashed border-slate-200 rounded-2xl p-3 bg-slate-50 hover:bg-slate-100 flex flex-col items-center justify-center text-center overflow-hidden cursor-pointer">
                ...
                {imageUrl ? (
                  ...
                  <>
                    ...
                    <p className="text-xs font-bold text-slate-700">{isUploading ? 'Uploading...' : 'Click to upload fabric image'}</p>
                  </>
                )}
              </div>
            </div>
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-2xl p-3 flex flex-col gap-2">
              ...
              <div className="pt-2 border-t border-slate-200 flex justify-between text-xs font-mono">
                <span className="font-bold text-emerald-700">+ PKR {profitPerUnit.toLocaleString()}</span>
                <span className="font-bold text-slate-800">{markupMargin}% Margin</span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              ...
              <div>
                <label className="text-[10.5px] font-bold text-slate-700 block mb-1">Rack Location</label>
                <input type="text" value={shelfLocation} onChange={(e) => setShelfLocation(e.target.value)} placeholder="Rack B-4" className="w-full px-3 py-2 bg-white border border-slate-300 rounded-xl text-xs font-bold text-slate-900" />
              </div>
            </div>
          </div>
      ...
      {showPrintModal && (
        <BarcodePrinterModal
          product={{ name: title || 'New Cloth / Fabric Item', sku, barcode: sku, price: retailPrice }}
